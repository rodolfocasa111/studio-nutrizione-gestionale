    ...
    if apri_nuovo:
        ...
        with tab_sintesi:
            ...
            def crea_pdf_one_page():
                ...
                upload_pdf_su_storage(p_sel["id"], f"Sintesi_Visita_{date.today()}.pdf", pdf_bytes)
                return pdf_bytes
        ...
        with tab_consenso:
            st.markdown("#### 📑 Modulo di Consenso Informato & Privacy GDPR")
            st.caption("Conferma l'accettazione e scarica il modulo di consenso informato in formato PDF.")
            ...
            def genera_pdf_consenso():
                ...
                pdf.ln(10)
                pdf.cell(100, 6, "")
                pdf.cell(w_utile - 100, 6, "________________________________________ (Firmato in studio)", align='R')
                
                pdf_bytes = bytes(pdf.output())
                upload_pdf_su_storage(p_sel["id"], f"Consenso_Privacy_{date.today()}.pdf", pdf_bytes)
                return pdf_bytes

            consenso_accettato = st.checkbox("✅ Il paziente ha letto l'informativa e autorizzato il trattamento dei dati e della prestazione sanitaria.", value=True)

            if consenso_accettato:
                st.download_button(
                    "📄 Scarica Modulo Consenso & Privacy (PDF)",
                    genera_pdf_consenso(),
                    file_name=f"Consenso_Privacy_{p_sel['cognome']}_{p_sel['nome']}.pdf",
        ...
        with col_btn_pdf:
            ...
            if not df_dieta.empty:
                ...
                def genera_pdf_completo():
                    ...
                    pdf_bytes = bytes(pdf.output())
                    upload_pdf_su_storage(paziente["id"], f"Piano_Nutrizionale_{date.today()}.pdf", pdf_bytes)
                    return pdf_bytes
                    ...
                    for p_nome in st.session_state["elenco_pasti"]:
                        ...
                        with st.form(f"form_add_{g}_{p_nome}", clear_on_submit=True):
                            ...
                            if invia and scelta_al:
                                ...
                                supabase.table("voci_dieta").insert({
                                    "dieta_id": dieta["id"], "giorno_settimana": g, "pasto": p_nome, "alimento_id": al_obj["id"], "grammi": quantita
                                }).execute()
                                st.rerun()
    ...
    with tab_fattura:
        ...
        if not pazienti_list:
            ...
            def crea_pdf_fattura():
                ...
                pdf_bytes = bytes(pdf.output())
                upload_pdf_su_storage(paz_fat["id"], f"Fattura_{num_fat}_{date.today()}.pdf", pdf_bytes)
                return pdf_bytes
